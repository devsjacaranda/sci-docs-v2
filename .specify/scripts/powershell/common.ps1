...
function Get-SpecifyDir {
    param([string]$RepoRoot = (Get-RepoRoot))
    return Join-Path (Get-Civ2DocsRoot -RepoRoot $RepoRoot) '.specify'
}
...
function Find-SpecifyRoot {
    ...
    while ($true) {
        ...
        if (Test-Path -LiteralPath (Join-Path (Join-Path $current $script:Civ2DocsDir) ".specify") -PathType Container) {
            return $current
        }
...
function Test-FeatureJsonMatchesFeatureDir {
    ...
    $featureJson = Join-Path (Get-SpecifyDir -RepoRoot $RepoRoot) 'feature.json'
...
function Get-InvokeSeparator {
    ...
    $separator = '.'
    $integrationJson = Join-Path (Get-SpecifyDir -RepoRoot $RepoRoot) 'integration.json'
...
function Resolve-Template {
    ...
    $base = Join-Path (Get-SpecifyDir -RepoRoot $RepoRoot) 'templates'
    ...
    $presetsDir = Join-Path (Get-SpecifyDir -RepoRoot $RepoRoot) 'presets'
    ...
    $extDir = Join-Path (Get-SpecifyDir -RepoRoot $RepoRoot) 'extensions'
    if (Test-Path $extDir) {
